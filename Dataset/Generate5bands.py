...
def compute_ndvi(nir_scaled_path, red_scaled_path, output_path):

    with rasterio.open(nir_scaled_path) as nir_src:
        nir = nir_src.read(1).astype(np.float32)
        meta = nir_src.meta.copy()

    with rasterio.open(red_scaled_path) as red_src:
        red = red_src.read(1).astype(np.float32)

    ndvi = (nir - red) / (nir + red + 1e-8)
    ndvi = np.clip(ndvi, -1.0, 1.0)

    ndvi_uint16 = ((ndvi + 1.0) / 2.0 * target_max).astype(np.uint16)

    meta.update(dtype="uint16", count=1)

    with rasterio.open(output_path, 'w', **meta) as dst:
        dst.write(ndvi_uint16, 1)

    print(f" NDVI genrado -> {os.path.basename(output_path)}")
    ...
    for zip_path in zip_files:
        ...
        try:
            with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                ...
                ndvi_path = os.path.join(temp_directory, f"{base_name}_NDVI_scaled.tif")
                compute_ndvi(
                    nir_scaled_path=scaled_files[3],  # NIR
                    red_scaled_path=scaled_files[0],  # Red
                    output_path=ndvi_path
                )
                scaled_files.append(ndvi_path)
                ...
                # Añadimos los archivos escalados (R, G, B, NIR, NDVI)
                merge_cmd.extend(scaled_files)
                merge_cmd.extend(["-o", output_file])
